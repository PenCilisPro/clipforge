 ...
 *
 * The worker was written against supabase-js (`from().select().eq()...`).
 * Rather than rewriting every call site, this shim implements the subset of
 * that API the app actually uses, backed by Firestore collections named
 * after the old tables. Worker only — runs with admin credentials.
 *
...
function parseCondition(cond) {
  ...
  return toFilter(field, op, value);
}
...
function toFilter(field, op, value) {
  switch (op) {
    ...
    case "lte":
      return Filter.where(field, "<=", value);
    case "in":
      return Filter.where(field, "in", Array.isArray(value) ? value : [value]);
    default:
...
class FirestoreQuery {
  ...
  or(expr) {
    this.filters.push({ make: () => parseOrFilter(expr) });
    return this;
  }
  ...
  async _queryRows(maxFetch = 2000) {
    ...
    const upper = this.rangeTo != null ? this.rangeTo + 1 : this.limitN ?? maxFetch;
    q = q.limit(upper);
    ...
    for (const { relation, arg } of embeds) {
      ...
      if (rel?.childOf) {
        ...
        for (let i = 0; i < parentIds.length; i += 30) {
          const snap = await db()
            .collection(relation)
            .where(rel.childOf, "in", parentIds.slice(i, i + 30))
            .get();
  ...
  async _run() {
    if (this._mutation) {
      ...
      // Support the `.insert(...).select("id").single()` chain some call
      // sites use: treat post-mutation selects as no-ops on the returned row.
      const chainable = {
    ...
    let rows = await this._queryRows();
  ...
  async _runMutation() {
    ...
    try {
      ...
      return { data: null, error: err("MUTATION_FAILED", e.message), status: 400, statusText: "Bad Request" };
    }
  }
}

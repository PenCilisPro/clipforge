...
4. **render** (per clip) — the worker hands the render provider a signed R2 source URL and trim offset; the provider cuts and renders the 1080×1920 clip with the caption track. `RENDER_PROVIDER=local` renders with ffmpeg on the worker itself (no webhook needed — the pipeline finalizes inline); the cloud providers (Creatomate/Shotstack) render remotely and complete via the required webhook (`RENDER_WEBHOOK_URL`).
